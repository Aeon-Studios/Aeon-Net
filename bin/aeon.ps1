...
<#
.SYNOPSIS
    Aeon.Net CLI - DNS Resolver Command Line Interface
.DESCRIPTION
    Command-line interface for the Aeon.Net DNS resolver.
    Provides simple commands for DNS operations.
.EXAMPLE
    aeon net -r example.com
    aeon net --resolve example.com -t MX
...
$modulePath = Join-Path $parentPath 'src'
$modulePath = Join-Path $modulePath 'Aeon.Net.psm1'

if (-not (Test-Path $modulePath)) {
    Write-Error "Aeon.Net module not found at $modulePath"
    exit 1
}
